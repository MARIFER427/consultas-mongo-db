...
public class GtController : Controller {
 [HttpGet("listar-pisos")]
    ...
    [HttpGet("listar-terreno")]
     public IActionResult ListarMetrosTerrreno(int metrosterreno){

        MongoClient client = new  MongoClient(CadenasConexion.MONGO_DB);
        var db = client.GetDatabase("Inmuebles");
        var collection = db.GetCollection<Inmueble>("RentasVentas");

        var filtro = Builders<Inmueble>.Filter.Eq(x => x.MetrosTerreno, metrosterreno);
...
[HttpGet("listar-costo")]
    ...
    [HttpGet("listar-fechapublicacion")]
     public IActionResult ListarFechaPublicacion(string fechapublicacion){
        ...
        var filtro = Builders<Inmueble>.Filter.Eq(x => x.FechaPublicacion, fechapublicacion);
